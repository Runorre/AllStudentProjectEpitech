...
from AIWithRef.src.const import (
    ELEVATION_REQUIREMENTS_STONES,
    RESOURCE_ABUNDANCE,
)
from AIWithRef.src.state import State
from AIWithRef.src.states import ConnectingState
...
class Context(object):
    ...
    def move_to_incantation(self, message: str) -> None:
        ...
        if (
            "need_players_for_incantation_" not in decrypted_message
            and "need_food" not in decrypted_message
        ):
            return
        parts: list[str] = message.split(", ")
        ...
        try:
            ...
            if "need_food" in decrypted_message and ((self.level <= 2 and self.lvl5 == True) or self.level == 1):
                self.drop_food()
            return
    ...
    def process_response(self, response: str) -> str:
        ...
        return self.receive_message2(self.is_helping)
    ...
    def fork(self) -> None:
        ...
        if nb == 0:
            if self.client_num >= 0:
                ...
                self.fork2()
                self.client_num -= 1
            elif int(self.inventory.get("food", 0)) <= 5:
                self.fork2()
                self.broadcast("need_food")
        else:
            Popen(
                [
                    "./zappy_ai",
